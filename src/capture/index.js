...
export async function captureFullPage(page, outputPath, options = {}) {
  const { onProgress, onStepEvent, onMeta, hideStickyAfterFirstFrame, frameDelay, maxHeight, pinOffset, pinOffsetPx, format = 'png', quality = 85, backdrop } = options;

  // Step 1 — OUT-01: scroll + per-viewport screenshots → ordered PNG Buffers + geometry.
  // maxHeight (v0.4 pin-format): when set, captureFrames clamps captureHeight to
  ...
  // pinOffset (v0.5/v0.6): fraction 0..1 of available vertical room. Works with
  // both pin and full-page captures — see captureFrames docs.
  // pinOffsetPx (v0.6): absolute-pixel alternative to pinOffset.
  const { frames, geometry } = await captureFrames(page, { onProgress, onMeta, hideStickyAfterFirstFrame, frameDelay, maxHeight, pinOffset, pinOffsetPx });
